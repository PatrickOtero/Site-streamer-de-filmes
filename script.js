...
function temaEscuro() {
  ...
  body.style.setProperty(
    "--shadow-color",
    "0px 4px 8px rgba(256, 256, 256, 0.15)"
  );
...
function geradorPaginas(array, arrayCorpo) {
  const arrayFiltrado = arrayCorpo.results.filter(
    (dados) =>
      dados.adult === false &&
      dados.title &&
      dados.poster_path &&
      dados.vote_average > 0
  );
...
function exibirModal() {
  ...
  movieNode.forEach((movie) => {
    movie.addEventListener("click", async function () {
      ...
      const resTratada5 = await (
        await fetch(
          "https://tmdb-proxy.cubos-academy.workers.dev/3/movie/" + filmeId
        )
      ).json();
...
async function requisicao() {
  ...
  destaqueLink.href =
    "https://www.youtube.com/watch?v=" + resTratada4.results[0].key;
}
